...
def evaluate(model, metric, data_loader):
    ...
    print("accu: %f" % (accu))
    print('in evaluate, calling train')
    model.train()  # Switch the model to training mode after evaluation


@paddle.no_grad()
def predict(model, data_loader):
    print('in predict, calling eval')
    model.eval()
    print('in predict, returned from eval')
    res = {}
    label_list = ['0', '1']
    print('pt 0')
    for batch in data_loader:
        ...
        for i in range(len(preds)):
            ...
            res[str(qas_id[i])] = label_list[preds[i]]
    print('in predict, calling train')
    model.train()
    return res
...
def old_read_row(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for id,line in enumerate(f):
            ...
            yield {'id': str(id), 'question': word1, 'answer': word2, 'antmorph': antmorph, 'labels': int(gold) }
...
def do_train(args):
    ...
    train_ds = load_dataset(read_row, data_path=dir + 'tag-' + args.pos + '-pairs.train.csv', lazy=False)
    dev_ds = load_dataset(read_row, data_path=dir + 'tag-' + args.pos + '-pairs.val.csv', lazy=False)
    test_ds = load_dataset(read_row, data_path=dir + 'tag-' + args.pos + '-pairs.test.csv', lazy=False)
    ...
    train_batch_sampler = paddle.io.DistributedBatchSampler(
        train_ds, batch_size=args.batch_size, shuffle=False) # changed from True by kwc
    ...
    dev_batch_sampler = paddle.io.BatchSampler(
        dev_ds, batch_size=args.batch_size, shuffle=False) # changed from False by kwc
    ...
    test_batch_sampler = paddle.io.BatchSampler(
        test_ds, batch_size=args.batch_size, shuffle=False) # changed from False by kwc
    ...
    model = model_class.from_pretrained(
        args.model_name_or_path, num_classes= 2 # len(train_ds.label_list)
        )
    ...
    for epoch in range(num_train_epochs):
        for step, batch in enumerate(train_data_loader):
            ...
            if global_step % args.save_steps == 0 or global_step == num_training_steps:
                if rank == 0:
                    print('calling evaluate on dev set')
